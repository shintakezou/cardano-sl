...
if [[ $core == true ]]; then
  stack build --nix --ghc-options="+RTS -A256m -n2m -RTS" $commonargs $norun --dependencies-only $args core/
  stack build --nix --ghc-options="+RTS -A256m -n2m -RTS" $commonargs $norun --fast $args 2>&1 core/ | perl -pe "$xperl" | grep -E --color "$xgrep"
else
  stack build --nix --ghc-options="+RTS -A256m -n2m -RTS" $commonargs $norun $webwallet --dependencies-only cardano-sl $args
  stack build --nix --ghc-options="+RTS -A256m -n2m -RTS" $commonargs $norun $webwallet --fast $args cardano-sl 2>&1 | perl -pe "$xperl" | grep -E --color "$xgrep"
fi

if [[ $test == true ]]; then
  stack build --nix --ghc-options="+RTS -A256m -n2m -RTS" $commonargs $webwallet --no-run-benchmarks --fast $args cardano-sl
fi
